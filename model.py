...
def DualResNet_hookmotion(cfg, pretrained=False):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=1, planes=32, spp_planes=128, head_planes=64,
                       augment=True)
    if pretrained:
        pretrained_state = torch.load(cfg.MODEL.PRETRAINED, map_location='cpu')
        model_dict = model.state_dict()
...
if __name__ == '__main__':
    ...
    cfg = config
    cfg.defrost()
    cfg.MODEL.PRETRAINED = "./pretrained_models/best_val_smaller.pth"
    x = torch.rand(1, 3, 1080, 1920).to(device=device)  # hardcoded hookmotion video resolution
    ...
    print(" fps of hookmotion video = {} fps".format(30))
